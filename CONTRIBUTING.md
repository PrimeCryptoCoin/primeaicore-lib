Contributing to primeaicore
=======

We're working hard to make *primeaicore* the most powerful JavaScript library for working with primeaicoin. Our goal is to have *primeaicore* be a library that can be used by anyone interested in primeaicoin, and to level expertise differences with great design and documentation.
...
These are some global design goals in primeaicore that any change must adhere.
...
When possible, primeaicore objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of primeaicore methods. For example, calling `JSON.stringify` on a primeaicore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'primeaicore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the primeaicore object.

DO:
```javascript
var primeaicore = require('../');
var PublicKey = primeaicore.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add underdarkskies git@github.com:github.com/PrimeCryptoCoin/primeaicore-lib.git
...
```
Finally go to [github.com/PrimeCryptoCoin/primeaicore-lib/primeaicore-lib](https://github.com/PrimeCryptoCoin/primeaicore-lib) in your web browser and issue a new pull request.
...
[github.com/PrimeCryptoCoin/issues](https://github.com/PrimeCryptoCoin/primeaicore-lib/issues).
