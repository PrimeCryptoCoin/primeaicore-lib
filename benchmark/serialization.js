...
async.series([
  function(next) {
    ...
    for (var i = 0; i < 100; i++) {
      ...
      // uint64le
      var br = new primeaicore.encoding.BufferWriter();
      var num = Math.round(Math.random() * 10000000000000);
      br.writeUInt64LEBN(new primeaicore.crypto.BN(num));
      buffers.push(br.toBuffer());

      // hashes
      var data = primeaicore.crypto.Hash.sha256sha256(new Buffer(32));
    ...
    function readUInt64LEBN() {
      ...
      var buf = buffers[c];
      var br = new primeaicore.encoding.BufferReader(buf);
  ...
  function(next) {
    ...
    function primeaicoreTest() {
      block1 = primeaicore.Block.fromString(blockData);
    }
    ...
    var suite = new benchmark.Suite();
    suite.add('primeaicore', primeaicoreTest, {maxTime: maxTime});
