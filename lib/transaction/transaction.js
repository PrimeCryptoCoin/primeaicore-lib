...
/**
 * Retrieve a hexa string that can be used with primeaid's CLI interface
 ...
 * * `disableMoreOutputThanInput`: disable checking if the transaction spends more primeaicoins than the sum of the input amounts
 * @return {string}
 */
 ...
 *
 * Can receive, as output information, the output of primeaid's `listunspent` command,
 * and a slightly fancier format recognized by primeaicore:
 *
 * ```
 * {
 ...
 * }
 * ```
 * Where `address` can be either a string or a primeaicore Address object. The
 * same is true for `script`, which can be a string or a primeaicore Script.
 *
 ...
 *
 * // From a pay to public key hash output from primeaid's listunspent
 ...
 * CheckTransaction in primeaicoin core.
 */
...
/**
 * Analogous to primeaid's IsCoinBase function in transaction.h
 */
